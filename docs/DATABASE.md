...
| Policy authoring result | `CUSTOMER#{customer_id}` | `POLICY_SOURCE#{source_id}#VERSION#{version}#{CANDIDATE\|UNSUPPORTED\|REJECTED\|UNCLASSIFIED}#{digest}` | 결과 하나당 item 하나. key는 Requirement의 결정적 digest이므로 worker 재시도가 같은 후보를 두 번 만들지 않는다. `UNCLASSIFIED`는 후보가 아니라 추출이 답하지 못한 unit이며(ADR-0025), digest는 locator 집합에서 유도한다. 네 segment 모두 `AUTHORING_RESULT_SEGMENTS`에 있어야 한다 — 빠지면 write-back 검증이 방금 쓴 item을 못 찾아 실행 전체가 fail-closed한다 |
