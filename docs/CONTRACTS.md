...
`packages/contracts/assessments.py`는 V3 평가 단계(`INITIAL`, `DEPLOYMENT_READINESS`, `POST_DEPLOY_VERIFICATION`)와 `EvaluationResult`의 기본 검증을 제공한다. Score 산출 자체는 AI Evaluator가 담당하며 Contract는 범위와 구조만 검증한다.
...
`GoldenDatasetCase`는 case ID, Assessment Phase, Resource Snapshot Artifact ID,
