...
5. After the foundation succeeds, validate the committed catalog plan from the
   ...
   `--registry isms-p-2023` (ADR-0026). It writes 101 MANUAL rules, 15
   automatable rules that cite the 11 technical items they are evidence for,
   and `profile-isms-p-baseline@v2` (120 items, one of which — the
   `isms-p-2023` source — already exists from the legacy registry and must
   match byte-for-byte). Customers never upload the standard; reviewers pick
   this profile as the `baseline` when publishing their own Profile. The
   command is idempotent: a second run reports every item as already matched.
   When the registry declares a newer profile version, the earlier version
   item stays and only the current pointer moves (conditionally); a partition
   whose pointer moved concurrently makes the run stop — re-run it.
