...
**상태: 철회 (2026-09-05).** 구현(`1857d08`)은 같은 날 되돌렸다. 두 가지 이유다.

1. 이 결정이 풀려던 문서(ISMS-P 점검표)는 ADR-0026으로 **추출 경로 자체를 쓰지 않게** 됐다 —
   인증기준은 운영자 기준선으로 게시된다. 남은 사내 문서(20 unit, 4 chunk)는 전부-아니면-전무
   게이트 아래에서도 완주해 왔다.
2. 구현이 회귀를 냈다. READY manifest의 count 집합에 `unclassified`를 **필수**로 더했는데, 그
   이전에 저장된 manifest에는 그 key가 없어 계약이 거부했고, 잘 되던 사내 문서 두 건의 후보
   조회가 `503 EXECUTION_ERROR`(`policy authoring manifest is invalid`)로 실패했다. 라이브 재실행
   측정(chunk 실패율 19–25%)은 또 다른 전부-아니면-전무 게이트(문서당 150개 상한)에 걸려
   아무것도 저장하지 못했다.

아래 본문은 결정 당시 그대로다. "실패한 chunk를 값으로 나른다"는 방향 자체는 여전히 유효하며,
큰 사내 문서가 다시 문제가 되면 **저장 계약을 additive로**(없는 count는 0으로 읽는다) 다시
구현해야 한다.
