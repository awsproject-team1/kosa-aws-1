...
## 보완 2026-09-05 — §2: 실행 오류는 점수를 막지 않고 옆에 실린다

§2의 "판정된 좌표만 평균하고 미판정은 따로 센다"가 `EXECUTION_ERROR`에는 적용되지 않고 있었다.
`calculate_readiness_score`는 실행 오류 좌표를 completed 집합에서 빼 계획을 미완료로 보았고, 그
결과 모델의 형식 실패 한 건이 판정된 좌표 28건의 점수를 통째로 숨겼다("계산 불가"). Coverage는
같은 좌표를 실행됨으로 세어 146/146을 보였으므로 두 화면이 모순됐다.

실행 오류는 조용한 누락이 아니다 — runner가 사유까지 결과로 남긴다(보이는 미완료). 그래서 완료로
보고, 평균에서 빼고, `ReadinessScore.errored_evaluations`로 수를 싣는다. 점수를 막는 것은 결과
자체가 없는 planned 좌표뿐이다. 화면은 "판정 N · 미판정 M · 실행 오류 K 제외"로 분모를 다 말한다.
