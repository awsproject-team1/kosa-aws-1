...
- **점수가 유의한지 측정하고, 개선 시도를 하나 되돌렸다 (2026-09-05).**
  ...
  - **점수는 status의 재진술이다.** 24 Case × 3회 = 72회에서 관측된 score는 **0과 100뿐**이고
    Case별 range는 전부 0이다. 자기일치율 1.0, 전이 방향 9/9 정상. 즉 재현성과 방향성은 있으나
    **연속 점수가 등급을 담지 않는다.** score를 "얼마나 지켰는가"로 읽으면 안 되고, 의미가 있는
    연속값은 코드가 severity 가중 평균으로 내는 Readiness Score다.
  - **status 정확도 21/24 (88%)이고 오류는 모두 한 방향이다.** 틀린 3건 전부 위반을 PASS로 본
    false negative이며, 부분 준수 Case에 집중된다(partial-compliance 2/4). 라이브에서는 반대
    방향 오탐도 관측했다 — AES256이 적용된 버킷을 암호화 없음으로 판정.
  - **되돌린 개선.** Catalog가 이미 선언한 근거 위치(`document_paths`)를 prompt에 넣어 "어느
    field가 이 Rule의 대상인지"를 알려 주는 변경을 만들고 A/B로 측정했다. 암호화 오탐은 실제로
    고쳐졌지만(FAIL→PASS), 모델이 그 **경로를 근거 reference로 인용**해 근거 게이트가 응답을
    통째로 거부하는 일이 급증했다(정확도 2/4 → 0/4). 게이트를 무르게 해 통과시키는 선택지는
    취하지 않고 변경을 되돌렸다.
  - **다음 후보는 prompt가 아니라 결정적 판정이다.** `document_paths`가 값의 위치를 이미 알고
    있으므로 "네 플래그가 모두 true인가" 같은 술어는 모델 없이 코드가 답할 수 있다. 부분 준수
    false negative가 사라지는 방향이고, 점수 입도보다 우선순위가 높다.
...
- **정책 추출이 긴 문서에서 거의 항상 실패하던 원인을 찾아 두 가지를 고쳤다 (2026-09-04).**
  ...
  - **원인 1 — Catalog 경계를 두 곳에서 다른 무게로 판정했다.** `build_candidate`는 위반마다
    코드를 붙여 후보 하나를 거절하는데 추출기는 같은 것을 예외로 올려 청크 전체를 죽였다.
    193 unit 문서의 39 청크 중 **13개가 오직 이 이유로** 실패했고, 그 청크에 함께 있던 멀쩡한
    요구사항까지 사라졌다. 추출기에서 그 검사를 걷어내 경계 판정을 `build_candidate` 한 곳에
    두었다. 경계는 그대로다 — 그런 후보는 여전히 승인 가능한 Rule이 되지 못하고, 이제는
    사라지는 대신 사유 코드와 함께 보존된다.
  - **원인 2 — 청크 실패가 한 번의 응답으로 확정됐다.** 게이트는 청크마다 걸리고 문서는 청크
    하나만 실패해도 실패하므로, 39 청크 문서는 청크 성공률이 조금만 낮아도 거의 확실히
    실패한다. 이제 청크를 최대 3회 묻고, locator 회계가 틀린 경우에는 무엇이 틀렸는지 이름으로
    되돌려 묻는다. 게이트는 그대로이고 부분 결과는 여전히 저장되지 않는다.
  - **결과.** 20 unit 참고 문서 실행 성공률 2/8 → **5/5**(자동 평가 Rule 17개). 193 unit 고객
    문서 실패 청크 18/39 → **6/39**.
  - **남은 문제.** 193 unit 문서는 아직 통과하지 못한다. 남은 6건 중 3건은 모델이 `SECTION`
    소제목을 두 목록 어디에도 넣지 않는 것이고(locator를 이름으로 알려줘도 반복된다), 3건은
    응답 모양 오류다. 소제목은 파서가 이미 구조로 알고 있는 정보라 서버가 처리할 수 있지만,
    "모델이 설명하지 않은 unit을 서버가 비요구사항으로 기록한다"는 것은 #88의 완결성 계약을
    바꾸는 결정이라 사용자 판단으로 남긴다.
...
- **`REVIEW_REQUIRED`를 통과할 문을 만들었다 (2026-09-05).** 라이브에서 ISMS-P 엑셀
  ...
  - Worker도 `needs_review` 요청을 지워진 문서와 같이 건너뛴다. 사람의 확인은 API로만 오고
    ...
    돼 있었다).
  - **콘솔에도 그 문을 냈다.** 백엔드만 고쳤을 때 사용자에게는 아무것도 달라지지 않았다 —
    업로드 화면이 `REVIEW_REQUIRED`를 모른 채 곧바로 추출을 요청했고, 그 요청은 영원히 실패하며
    화면에는 "후보 추출이 2분 내에 완료되지 않았습니다"로만 보였다. 이제 정규화 직후 그 상태를
...
- **사용자당 여러 Profile 지정 · "지원 예정" 카드 (2026-09-05).**
  ...
  - "지원 예정" 좌표가 여전히 `MANUAL_REVIEW` 배지와 "조치 요청" 버튼을 달고 있었다. 사람 검토
    ...
    말한다.
  - **점수 분모를 다 적는다 · DRIFT PASS는 "일치" (2026-09-05).** 준비도 13이 "PASS/FAIL 비율이
    비슷한데 왜 이렇게 낮나"로 읽혔다. 결과표 PASS 17건 중 12건이 DRIFT PASS였다 — 준수가 아니라
    "IaC와 AWS 판정이 같다"이고 둘 다 FAIL이어도 일치다. 배지를 "일치"(회색)로 바꾸고 PASS 집계에서
